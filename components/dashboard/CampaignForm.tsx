...
import { useCreateCampaign, useFetchCampaign, useFetchProductsByStore, useFetchStores, useUpdateCampaign } from "@/hooks/useCampaignAndPromotions";
...
export default function CampaignForm({ campaignId }: { campaignId?: string }) {
  ...
  const { campaign, isLoading: isFetchingCampaign } = useFetchCampaign(campaignId);
  ...
  const [storeOption, setStoreOption] = useState<any>(null);
  ...
  useEffect(() => {
    if (campaign && isEditMode) {
      ...
      if (campaign.imageUrl) {
        setImageUrls([campaign.imageUrl]);
      }
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    ...
    try {
      if (isEditMode) {
        await updateCampaign({ id: campaignId!, data: finalValues });
     
      } else {
        await createCampaign(finalValues);
      }
      // router.push("/dashboard/marketing?tab=campaigns");
    } catch (error) {
      toast({
        title: "Error",
        description: `Failed to ${isEditMode ? "update" : "create"} campaign`,
        variant: "destructive",
      });
    }
  };
  ...
  if (isFetchingCampaign && isEditMode) {
    return (
  ...
  return (
    <div className="container mx-auto px-4 py-3">
      <Button
        variant="ghost"
        className="mb-6"
        onClick={() => router.push("/dashboard/marketing?tab=campaigns")}
      >
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-600 mt-1">
          {isEditMode
            ? "Update your campaign details and product selections"
            : "Set up a new campaign with products and promotional details"}
        </p>
      </div>
                      ...
                      render={({ field }) => (
                        <FormItem className="flex flex-row items-start space-x-3 space-y-0 rounded-md border p-4">
                          ...
                          <div className="space-y-1 leading-none">
                            ...
                            <FormDescription>
                              This campaign will be visible to customers if active
                            </FormDescription>
                          </div>
                        </FormItem>
                      )}
