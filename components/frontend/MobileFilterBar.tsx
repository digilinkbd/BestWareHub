...
import { ArrowUpDown, Filter, X, Search, Minus } from "lucide-react"
import {
  Drawer,
  DrawerTrigger,
  DrawerContent,
...
const MobileFilterBar = ({
  ...
  const sortOptions = ["recommended", "price-low-high", "price-high-low", "newest"]
            ...
            <div className="space-y-2">
              {deliveryModes.map((mode) => (
                <div key={mode.id} className="flex items-center space-x-2">
                  <Checkbox
                    id={`mobile-delivery-${mode.id}`}
                    checked={currentParams.deliveryModes?.includes(mode.id)}
                    onCheckedChange={() => toggleFilter("delivery", mode.id)}
                  />
                  <label htmlFor={`mobile-delivery-${mode.id}`} className="text-sm cursor-pointer flex items-center">
            ...
            <div className="space-y-2">
              {deals.map((deal) => (
                <div key={deal.id} className="flex items-center justify-between">
                  <div className="flex items-center space-x-2">
                    <Checkbox
                      id={`mobile-deal-${deal.id}`}
                      checked={currentParams.deals?.includes(deal.id)}
                      onCheckedChange={() => toggleFilter("deal", deal.id)}
                    />
                    <label htmlFor={`mobile-deal-${deal.id}`} className="text-sm cursor-pointer">
                      {deal.label}
                    </label>
                  </div>
            ...
            <div className="space-y-2">
              {newArrivals.map((arrival) => (
                <div key={arrival.id} className="flex items-center space-x-2">
                  ...
                  />
                  <label htmlFor={`mobile-arrival-${arrival.id}`} className="text-sm cursor-pointer">
                    {arrival.label}
                  </label>
                </div>
              ))}
                ...
                {isVendorsLoading ? (
                  ...
                  filteredVendors.map((vendor) => (
                    <div key={vendor.id} className="flex items-center justify-between">
                      <div className="flex items-center space-x-2">
                        <Checkbox
                          id={`mobile-seller-${vendor.id}`}
                          checked={currentParams.sellers?.includes(vendor.id)}
                          onCheckedChange={() => toggleFilter("seller", vendor.id)}
                        />
                        <label htmlFor={`mobile-seller-${vendor.id}`} className="text-sm cursor-pointer">
                          {vendor.title}
                        </label>
                      </div>
  ...
  return (
    <div className="fixed bottom-16 left-1/2 transform -translate-x-1/2 p-4 z-50 flex gap-2">
      {/* Filter Button with Drawer */}
      <Drawer open={drawerOpen} onOpenChange={setDrawerOpen}>
        <DrawerTrigger asChild>
          <Button className="bg-blue-600 hover:bg-blue-700 text-white shadow-lg px-6 flex items-center gap-2">
            <Filter className="h-4 w-4" />
            Filter
            {selectedFilters.length > 0 && (
              <Badge className="bg-white text-blue-600 ml-1 h-5 flex items-center justify-center rounded-full">
        ...
        <DrawerContent className="h-[80vh] overflow-y-auto">
          ...
          <DrawerFooter className="border-t sticky bottom-0 bg-white z-10">
            <Button className="w-full bg-blue-600 hover:bg-blue-700" onClick={() => setDrawerOpen(false)}>
      ...
      <Sheet>
        ...
        <SheetContent side="bottom" className="p-6 rounded-t-2xl shadow-xl">
          <h2 className="text-lg font-semibold text-center text-gray-900">Sort By</h2>
          <RadioGroup
          ...
          >
            {sortOptions.map((option, index) => (
              <div key={index} className="flex items-center gap-3">
                <RadioGroupItem id={`sort-${index}`} value={option} />
                <Label htmlFor={`sort-${index}`} className="text-sm">
                  {sortLabels[option]}
                </Label>
              </div>
            ))}
