...
export default function NotFound() {
  useEffect(() => {
    ...
    const createConfetti = () => {
      ...
      for (let i = 0; i < 50; i++) {
        ...
        confetti.style.backgroundColor = ['#FCD34D', '#FBBF24', '#F59E0B', '#EAB308'][Math.floor(Math.random() * 4)];
  ...
  return (
    <div className="bg-gradient-to-br from-yellow-50 to-amber-100 min-h-screen overflow-hidden relative">
      <div id="confetti-container" className="absolute inset-0 pointer-events-none" />
      
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <motion.div 
          className="relative h-[calc(100vh-85px)] flex justify-center items-center flex-col"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
        ...
        >
          ...
          >
            ...
            >
              ...
              <path
                ...
                fill="#F59E0B"
              />
              <circle cx="136.191" cy="31.0854" r="1.36752" fill="#F59E0B" />
              <circle cx="141.661" cy="31.0854" r="1.36752" fill="#F59E0B" />
              <circle cx="147.131" cy="31.0854" r="1.36752" fill="#F59E0B" />
            </svg>
          </motion.div>
          
          <motion.div 
            className="block text-center mt-8 max-w-xl"
          ...
          >
            ...
            <motion.h2 
              className="md:text-2xl text-xl leading-8 text-gray-900 font-medium mb-3"
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              transition={{ delay: 0.5, duration: 0.8 }}
            >
              <span className="text-amber-600 font-semibold">Oops!</span> You've ventured into uncharted territory
            </motion.h2>
            ...
            >
              The page you're looking for seems to have gone on an adventure. <br />
              Don't worry, we're working to bring it back!
            </motion.p>
            
            <motion.div 
              className="flex flex-col gap-4 sm:flex-row sm:justify-center py-6"
              initial={{ y: 20, opacity: 0 }}
              animate={{ y: 0, opacity: 1 }}
            ...
            >
              <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                <Button 
                  variant="default" 
                  asChild
                  className="bg-amber-500 hover:bg-amber-600 text-white border-none shadow-lg px-6 py-5"
                >
                  <Link href="/">
              ...
              <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                <Button 
                  ...
                  className="border-amber-400 text-amber-600 hover:bg-amber-50 px-6 py-5"
                >
